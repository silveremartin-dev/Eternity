...
public abstract class AbstractEternityBoard implements EternityBoardInterface {
    ...
    protected AbstractEternityBoard(int maxXValue, int maxYValue) {
        if (maxXValue > 0) {
            if (maxYValue > 0) {
                ...
                throw new IllegalArgumentException("Coordinate Y must be greater than 0.");
        } else
            throw new IllegalArgumentException("Coordinate X must be greater than 0.");
    }
    ...
    public boolean areBordersCorrect() {
        ...
        while (j < (maxXValue - 1) && result) {
            result = (tiles[0 + j * maxYValue].getLeft().equals(AbstractEternityBasicPatterns.EternityBasicPatternGray)
                    || tiles[(maxXValue - 1) + j * maxYValue].getRight()
                            .equals(AbstractEternityBasicPatterns.EternityBasicPatternGray));
            j++;
        }
        i = 0;
        j = maxYValue - 1;
        while (i < maxXValue && result) {
            result = tiles[i + j * maxYValue].getBottom()
                    .equals(AbstractEternityBasicPatterns.EternityBasicPatternGray);
            i++;
        }
        return result;
    ...
    public boolean isWinningSolution() {
        return computeScore() == (4 * 2 + ((maxXValue - 2) * 2 + (maxXValue - 2) * 2) * 3
                + (maxXValue - 2) * (maxYValue - 2) * 4) / 2;
    }
    ...
    public boolean isTileFree(int x, int y) {
        return tiles[x + maxYValue * y] == null
                || tiles[x + maxYValue * y].equals(AbstractEternityTiles.EternityTile0);
    }
    ...
    public boolean areNeighborsMatching(int x, int y, @NotNull EternityTileInterface eternityTile) {
        ...
        boolean result;
        // left tile if any
        result = (x > 0 && (tiles[(x - 1) + maxYValue * y] == null || (tiles[(x - 1) + maxYValue * y] != null
                && eternityTile.getLeft().equals(tiles[(x - 1) + maxYValue * y].getRight()))));
        // right tile if any
        result = result && (x < (maxXValue - 1))
                && (tiles[(x + 1) + maxYValue * y] == null || (tiles[(x + 1) + maxYValue * y] != null
                        && eternityTile.getRight().equals(tiles[(x + 1) + maxYValue * y].getLeft())));
        // top tile if any
        result = result && (y > 0 && (tiles[x + maxYValue * (y - 1)] == null || (tiles[x + maxYValue * (y - 1)] != null
                && eternityTile.getTop().equals(tiles[x + maxYValue * (y - 1)].getBottom()))));
        // bottom tile if any
        result = result && (y < (maxYValue - 1))
                && (tiles[x + maxYValue * (y + 1)] == null || (tiles[x + maxYValue * (y + 1)] != null
                        && eternityTile.getBottom().equals(tiles[x + maxYValue * (y + 1)].getTop())));
        return result;
    }
    ...
    public int getNeighborsMatchingCount(int x, int y, @NotNull EternityTileInterface eternityTile) {
        ...
        result = 0;
        // left tile if any
        if (x > 0 && tiles[(x - 1) + maxYValue * y] != null
                && eternityTile.getLeft().equals(tiles[(x - 1) + maxYValue * y].getRight())) {
            result++;
        }
        // right tile if any
        if (x < (maxXValue - 1) && tiles[(x + 1) + maxYValue * y] != null
                && eternityTile.getRight().equals(tiles[(x + 1) + maxYValue * y].getLeft())) {
            result++;
        }
        // top tile if any
        if (y > 0 && tiles[x + maxYValue * (y - 1)] != null
                && eternityTile.getTop().equals(tiles[x + maxYValue * (y - 1)].getBottom())) {
            result++;
        }
        // bottom tile if any
        if (y < (maxYValue - 1) && tiles[x + maxYValue * (y + 1)] != null
                && eternityTile.getBottom().equals(tiles[x + maxYValue * (y + 1)].getTop())) {
            result++;
        }
        return result;
    ...
    public boolean areBordersMatchingForBorderTile(int x, int y, @NotNull EternityTileInterface eternityTile) {
        boolean result;
        result = ((x == 0) && eternityTile.getLeft().equals(AbstractEternityBasicPatterns.EternityBasicPatternGray))
                || ((x == (maxXValue - 1))
                        && eternityTile.getRight().equals(AbstractEternityBasicPatterns.EternityBasicPatternGray));
        result = result && (((y == 0)
                && eternityTile.getTop().equals(AbstractEternityBasicPatterns.EternityBasicPatternGray))
                || ((y == (maxYValue - 1))
                        && eternityTile.getBottom().equals(AbstractEternityBasicPatterns.EternityBasicPatternGray)));
        return result;
    }

    public boolean areBordersMatchingForInBoardTile(@NotNull EternityTileInterface eternityTile) {
        return !eternityTile.getLeft().equals(AbstractEternityBasicPatterns.EternityBasicPatternGray)
                && !eternityTile.getRight().equals(AbstractEternityBasicPatterns.EternityBasicPatternGray)
                && !eternityTile.getTop().equals(AbstractEternityBasicPatterns.EternityBasicPatternGray)
                && !eternityTile.getBottom().equals(AbstractEternityBasicPatterns.EternityBasicPatternGray);
    }
