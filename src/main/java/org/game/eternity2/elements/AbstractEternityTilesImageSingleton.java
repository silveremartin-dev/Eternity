...
public class AbstractEternityTilesImageSingleton {
    ...
    protected AbstractEternityTilesImageSingleton() {
        tilesImage = new Image[AbstractEternityTiles.TILESSTORE.getXBoardSize()
                * AbstractEternityTiles.TILESSTORE.getYBoardSize()];
        for (int i = 0; i < AbstractEternityTiles.TILESSTORE.getXBoardSize(); i++) {
            for (int j = 0; j < AbstractEternityTiles.TILESSTORE.getYBoardSize(); j++) {
                tilesImage[i + j * AbstractEternityTiles.TILESSTORE.getYBoardSize()] = computeImageForTile(
                        AbstractEternityTiles.TILESSTORE.getTileAt(i, j));
            }
        }
    }
    ...
    public Image getImageForTile(@NotNull EternityTileInterface eternityTile) {
        return tilesImage[eternityTile.getBackValue()];
    }

    protected Image computeImageForTile(EternityTileInterface eternityTile) {
    ...
    // we assume there is 4 images, each, square and of the same size, in top, left,
    // bottom and right order
    private BufferedImage combineImages(BufferedImage... bufferedImages) {
        ...
        for (x = width / 2; x < width; x++) {
            y = x;
            while (y < (height - x)) {
                resultImage.setRGB(x, y, pixelsImage1.getRGBAsInt(x, y));
                y++;
            }
        }
        ...
        for (y = height / 2; y < height; y++) {
            x = height - y;
            while (x < y) {
                resultImage.setRGB(x, y, pixelsImage2.getRGBAsInt(x, y));
                x++;
            }
        }
        ...
        for (x = 0; x < width / 2; x++) {
            y = x;
            while (y < (height - x)) {
                resultImage.setRGB(x, y, pixelsImage3.getRGBAsInt(x, y));
                y++;
            }
        }
