...
public interface EternityBoardInterface extends EternityGameElementInterface, Serializable, Cloneable {
    ...
    // use at your own risks, same as getTileAt without the checks for speedup.
    // works better by calling other methods first to check if there is no tile at
    // this location and borders and neighbors match
    ...
    // checks if this is a border tile with correct orientation to be put on the
    // border or on the opposite that the tile has no border when it is to be put on
    // the inside Board
    // check if new tile matches with neighbors
    // parameter tile can be null though it is preferred to use
    ...
    // areHintTilesInPlace(EternityBoardReader.hintEternityBoard);
    boolean areHintTilesInPlace(@NotNull EternityBoardInterface hintsBoard);

    // returns true if the bord wasn't set for any tile where there should be an
    // hint tile
    // should be called with
    // areAllHintTilesFree(EternityBoardReader.hintEternityBoard);
    boolean areAllHintTilesFree(@NotNull EternityBoardInterface hintsBoard);

    // should be done before adding tiles
    // should be called with
    // putHintTilesOnBoardNoCheck(EternityBoardReader.hintEternityBoard);
    ...
    // should be called with
    // putHintTilesOnBoard(EternityBoardReader.hintEternityBoard);
    ...
    // check that the Board is completely filled with something else than
    // EternityBasicPatternGray, but the tilling may still be invalid
    boolean isBoardFilled();

    // should not happen if you pick the tiles to put on board one after another
    // from a Set of Tiles like the EternityTiles.TILESSTORE
