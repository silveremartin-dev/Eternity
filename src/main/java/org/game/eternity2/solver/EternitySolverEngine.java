...
public class EternitySolverEngine {
    ...
    public EternitySolverEngine(long[] allPieces, int width, int height) {
        ...
        if (allPieces != null) {
            for (long p : allPieces) {
                maxId = Math.max(maxId, PiecePrimitive.getId(p));
            }
        }
        this.pieceUsed = new boolean[maxId + 100];
    ...
    private void ensurePieceUsedCapacity(int id) {
        if (id >= pieceUsed.length) {
            boolean[] newArray = new boolean[id + 100];
            System.arraycopy(pieceUsed, 0, newArray, 0, pieceUsed.length);
            pieceUsed = newArray;
        }
    }
    ...
    public void loadState(BoardPrimitive startingBoard) {
        ...
        this.initialCursor = (firstEmpty == -1) ? totalCells : firstEmpty;
        ...
        this.bestPlacedCount = count;
        System.arraycopy(board, 0, bestBoard, 0, totalCells);
        this.bestScore = startingBoard.computeScore();
    }

    /**
     * Start the solving process.
     * Uses a hybrid approach: standard backtracking with periodic stochastic
     * restarts.
     */
    public void solve() {
        solve(false);
    ...
    private void solve(boolean stochastic) {
        ...
        while (cursor < totalCells) {
            ...
            for (int i = iterDesde[cursor]; i < candidates.length; i++) {
                ...
                ensurePieceUsedCapacity(pieceId);
                ...
                if (!pieceUsed[pieceId] && matchesBoard(piece, x, y)) {
                    ...
                    if (cursor + 1 > bestPlacedCount) {
                        bestPlacedCount = cursor + 1;
                        System.arraycopy(board, 0, bestBoard, 0, totalCells);
                        bestScore = getBestBoard().computeScore();
                        if (statistics != null)
                            statistics.updateBestBoard(getBestBoard());
                    }
            ...
            if (placed) {
                ...
                int removedId = PiecePrimitive.getId(removedPiece);
                ensurePieceUsedCapacity(removedId);
                pieceUsed[removedId] = false;
    ...
    public int getBestScore() {
        return getBestBoard().computeScore();
    }
}
