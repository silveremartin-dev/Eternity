...
public class EternityTiles6x12ImageSingleton extends AbstractEternityTilesImageSingleton {

    private static class EternityTilesImageSingletonHolder {
        private final static EternityTiles6x12ImageSingleton instance = new EternityTiles6x12ImageSingleton();

    }

    private static Image[] tilesImage;

    private EternityTiles6x12ImageSingleton() {
        tilesImage = new Image[72];
        for (int i=0; i<6;i++) {
            for (int j=0; j<12;j++) {
                tilesImage[i+j*12] = computeImageForTile(EternityTiles6x6.TILESSTORE.getTileAt(i, j));
            }
        }
    }

    public static EternityTiles6x12ImageSingleton getInstance() {
